...
-- Procedure: Process Money Transfer
...
-- Function: Calculate Transaction Velocity
...
BEGIN
    ...
    VALUES (UUID(), NEW.sender_id, NEW.amount, v_current_balance, NEW.payment_method, NEW.timestamp);
...
SELECT 
    ...
    COALESCE(SUM(t_sent.amount), 0) as total_sent,
    COALESCE(SUM(t_received.amount), 0) as total_received,
    COALESCE(MAX(GREATEST(COALESCE(t_sent.timestamp, '1970-01-01'), COALESCE(t_received.timestamp, '1970-01-01'))), '1970-01-01') as last_transaction_date,
...
CREATE OR REPLACE VIEW v_monthly_transaction_report AS
SELECT 
    year,
    month,
    transaction_count,
